...
def _resolve_val_path(cfg: TrainConfig) -> Path | None:
    ...
    if p.exists():
        return p
    # Proof-test workdir copies protocol eval/ (no private shard). The shard
    # from data.prepare lives next to the recipe data tree — resolve via manifest.
    manifest = Path(cfg.manifest_path)
    if manifest.is_file():
        fallback = manifest.resolve().parent.parent / "eval" / "private" / "active_tokens.bin"
        if fallback.exists():
            return fallback
    return None
